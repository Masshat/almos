...
inline void* ppm_page2addr(struct page_s *page)
{
	...
	ppm = &clusters_tbl[page->cid].cluster->ppm;
		 ...
		 struct boot_info_s *info)
{
	...
	ppm->ppn_start = begin >> PMM_PAGE_SHIFT;
	...
	ppm->pages_nr = (limit - begin) >> PMM_PAGE_SHIFT;
	ppm->free_pages_nr = 0;
	ppm->begin = begin;
...
static error_t ppm_init_finalize(struct ppm_s *ppm, struct boot_info_s *info)
{
	...
	for(reserved = info->reserved_start; reserved < info->reserved_end; reserved += PMM_PAGE_SIZE)
	{
		if(cid == info->boot_cluster_id)
			page = ppm_ppn2page(ppm, reserved >> PMM_PAGE_SHIFT);
		else
...
void ppm_print(struct ppm_s *ppm)
{
	...
	print("%s: Begin addr %x, Start addr %x\nFree pages number %d\n",
	      __FUNCTION__, 
	      ppm->begin, 
	      ppm->vaddr_start, 
	      ppm->free_pages_nr);
