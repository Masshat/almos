...
VFS_OPEN_FILE(sysfs_open)
{
	...
	if(entry->op.open)
		return entry->op.open(entry, &f_info->rq, &file->f_offset);
...
VFS_READ_FILE(sysfs_read)
{
	...
	{
		if((err = info->entry->op.read(info->entry, &info->rq, &file->f_offset)))
		{
	...
	memcpy(buffer, &info->rq.buffer[info->current_index], size);
...
VFS_READ_DIR(sysfs_readdir)
{
	...
	dirent->d_type = (metafs_hasChild(current)) ? VFS_DIR : 0;
