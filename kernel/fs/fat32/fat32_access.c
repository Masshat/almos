		       ...
		       vfat_cluster_t *next_cluster_index)
{
  ...
  if ((page = mapper_get_page(mapper, page_id, MAPPER_SYNC_OP, NULL)) == NULL)
    return -1;
  ...
  *new_cluster = 0;
  ...
  for(sector = last_allocated_sector; (sector < EOF_sector) && (!found); sector ++)
  {
    ...
    {  
      ...
      page = mapper_get_page(mapper, page_id, MAPPER_SYNC_OP, NULL);
			    ...
			    vfat_cluster_t *next_cluster)
{
  ...
  page = mapper_get_page(mapper, lba / sectors_per_page, MAPPER_SYNC_OP, NULL);
  ...
  else
  {
    ...
    if((page = mapper_get_page(mapper, lba / sectors_per_page, MAPPER_SYNC_OP, NULL)) == NULL)
...
error_t vfat_locate_entry(struct vfat_entry_request_s *rq)
{
  ...
  while(!found) 
  {
    page = mapper_get_page(mapper, current_page, MAPPER_SYNC_OP, NULL);
			    ...
			    uint_t *extended) 
{
  ...
  *extended = 0;
  ...
  mapper = ctx->mapper;
  page = mapper_get_page(mapper, old_page_id, MAPPER_SYNC_OP, NULL);
