...
#if 1
#define USR_OFFSET      (CONFIG_USR_OFFSET)
				...
				
{
  ...
  {   
    ...
    {
      err = EPERM;
      printk(ERROR, "\nERROR: %s: p_vaddr %x, index %d [ EPERM ]\n", __FUNCTION__, p_entry->p_vaddr, index);
      return err;
    }
#endif
    ...
    if(err == (error_t)VM_FAILED)
    {
      printk(WARNING,"WARNING: %s: Faild to map segment <0x%x - 0x%x>, proto %x, file name %x\n",
	     __FUNCTION__, start, limit, proto, file->f_node->n_name);
...
error_t elf_load_task(char *pathname, struct task_s *task)
{
  ...
  elf_dmsg(1, "%s: elf entry point @%x [%s]\n", __FUNCTION__, (uint_t) e_header->e_entry, pathname);
