...
int iprintk (char *addr, int inc, const char *fmt, va_list ap)
{
	...
	*addr = *fmt++;
      ...
      if (*fmt)
      {
	    ...
	    tmp = compute_dec(&addr, &buf[0], &count, inc, val, true); 
            break;
         case 'u':
	    uval = va_arg (ap, uint_t);
	    tmp = compute_dec(&addr, &buf[0], &count, inc, uval, false); 
            break;
	 case 'D':
	    val64 = va_arg (ap, int64_t);
	    tmp = compute_dec64(&addr, &buf[0], &count, inc, val64, true); 
	    break;
	 case 'U':
            val64 = va_arg (ap, uint64_t);
	    tmp = compute_dec64(&addr, &buf[0], &count, inc, val64, false); 
            break;
         case 's':
	    tmp = va_arg (ap, char *);
            if (!tmp)
