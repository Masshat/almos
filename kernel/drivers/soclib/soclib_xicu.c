...
{
	xicu_reg_write(icu->base, type, output_irq, mask);

	if(type == XICU_PTI_TYPE)
		xicu_reg_write(icu->base, XICU_PTI_PER, output_irq, CPU_CLOCK_TICK);
}
...
void xicu_timer_irq_handler (struct device_s *xicu, uint_t output_irq)
{
	...
	cpu = current_cpu;
	cpu_clock(cpu);
	xicu_reg_read(xicu->base, XICU_PTI_ACK, output_irq);
	xicu_reg_write(xicu->base, XICU_PTI_VAL, output_irq, cpu_get_ticks_period(cpu));
}
...
void xicu_irq_handler (struct irq_action_s *action)
{
	...
	if(XICU_PRIO_HAS_WTI(prio))
	{
		...
		(void)xicu_reg_read(xicu->base, XICU_WTI_REG, output_irq);
	...
	if(XICU_PRIO_HAS_CNTR(prio))
	{
		...
		while(irq_state)
		{
			if(irq_state & 0x1)
			{
				xicu_reg_write(xicu->base, XICU_CNTR_REG, irq_num, ctx->cntr_irq_vector[irq_num].value);
	
				if(ctx->cntr_irq_vector[irq_num].event != NULL)
					event_send(ctx->cntr_irq_vector[irq_num].event, &cpu->le_listner);
			}
...
error_t soclib_xicu_ipi_send(struct device_s *xicu, uint_t port)
{
	xicu_reg_write(xicu->base, XICU_WTI_REG, port, 0);
	return 0;
}
sint_t soclib_xicu_barrier_init(struct device_s *xicu, struct event_s *event, uint_t count)
{
	...
	if(hwid != -1)
	{
		...
		xicu_reg_write(xicu->base, XICU_CNTR_REG, hwid, ctx->cntr_irq_vector[hwid].value);
	}
...
error_t soclib_xicu_init(struct device_s *icu, void *base, uint_t size, uint_t irq)
{
	kmem_req_t req;
	struct xicu_context_s *ctx;
