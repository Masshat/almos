...
static sint_t fb_read(struct device_s *fb, dev_request_t *rq)
{
	...
	src   = ((uint8_t *) fb->base + rq->file->f_offset);
	...
	if((err = dma_memcpy(dst, src, size, DMA_ASYNC)))
		printk(ERROR, "ERROR: fb_write: faild to do DMA memcpy request, err %d\n", err);
#else
...
static sint_t fb_write(struct device_s *fb, dev_request_t *rq)
{
	...
	dst = ((uint8_t *) fb->base + rq->file->f_offset);
	limit = (uint8_t*)fb->base + (uint_t)fb->data;
	...
	if((err = pmm_get_page(&current_task->vmm.pmm, (vma_t)dst, &info)))
	{
		printk(ERROR, "ERROR: fb_write: faild to get dst page info, err %d\n", err);
		return -1;
	}
  
	dst = (uint8_t*)((info.ppn << PMM_PAGE_SHIFT) | ((vma_t)dst & PMM_PAGE_MASK));
	void *src = task_vaddr2paddr(current_task, rq->src);
