...
error_t ls_func(void *param)
{
  ...
  for(i=1; i < argc; i++)
  {
    ...
    if((err=vfs_opendir(ms_n_cwd,path_name,0,&dir)))
      return err;
    ...
    while(!(err=vfs_readdir(dir,&dirent)))
    {
      count++;
      show3();
 ...
 void show1(void)
{
  ...
  ksh_print("\tType: %s\n", (VFS_IS(dirent.d_type, VFS_DIR)) ? "<DIR>" : 
	   (VFS_IS(dirent.d_type, VFS_FIFO)) ? "FIFO file" : "Reg file");
  ksh_print("=============================\n");
}
