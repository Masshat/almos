...
static void kthread_exit(void)
{
	...
	printk(INFO,"INFO: Kernel Thread %x on CPU %d has finished\n", 
	       this, 
	       current_cpu->gid);
...
struct thread_s* kthread_create(struct task_s *task, kthread_t *kfunc, void *arg, uint_t cid, uint_t cpu_lid)
{
	kmem_req_t req;
