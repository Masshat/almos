...
int sys_cond_var(struct cv_s **cv, uint_t operation, struct semaphore_s **sem)
{
	...
	if((err = cpu_uspace_copy(&icv, cv, sizeof(struct cv_s *))))
		goto SYS_COND_END;
	...
	case CV_INIT:
		...
		if((err = cpu_uspace_copy(cv, &icv, sizeof(struct cv_s*))))
		{
			req.ptr = icv;
			kmem_free(&req);
		}
	...
	case CV_WAIT:
		...
		if((err = cpu_uspace_copy(&isem, sem, sizeof(struct semaphore_s *))))
			break;
