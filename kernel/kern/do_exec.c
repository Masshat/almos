...
error_t args_len(char **vect, uint_t pages_max, uint_t *pages_nr, uint_t *entries_nr)
{
	...
	while(1)
	{
		if((err = cpu_uspace_copy(&ptr, &vect[cntr], sizeof(ptr))))
		{
			printk(INFO, "INFO: %s: EFAULT Has been Catched, vect %x\n", __FUNCTION__, vect + cntr);
			return err;
		}
		     ...
		     uint_t *pgindex)
{
	...
	err = args_len(vect, pages_max, &pages_nr, &cntr);
	...
	*current = start - (pages_nr * PMM_PAGE_SIZE);
	...
	{
		err = cpu_uspace_copy((void*)paddr, vect + (pages_nr << PMM_PAGE_SHIFT), PMM_PAGE_SIZE);
	...
	if(count > 0)
	{
		err = cpu_uspace_copy((void*)paddr, vect + (pages_nr << PMM_PAGE_SHIFT), count);
		if(err) return err;
	}
	...
	for(i=0; i < cntr; i++)
	{
		...
		while(len)
		{
			...
			{
				err = cpu_uspace_copy((void*)paddr, ptr, len);
				if(err) return err;
			...
			{
				err = cpu_uspace_copy((void*)paddr, ptr, PMM_PAGE_SIZE - count);
				if(err) return err;
...
error_t task_load_init(struct task_s *task)
{
	...
	err = pmm_init(&init->vmm.pmm, attr.cluster);
	...
	vfs_node_up_atomic(task->vfs_root);
	init->vfs_root = task->vfs_root;
	vfs_node_up_atomic(task->vfs_root);
	init->vfs_cwd  = task->vfs_root;

	init->vmm.limit_addr = CONFIG_KERNEL_OFFSET;
	init->uid = 0;
	init->parent = task;
