...
{
	return soclib_xicu_barrier_init(cluster->arch.xicu, event, count);
}

sint_t arch_barrier_wait(struct cluster_s *cluster, uint_t barrier_id)
{
	return soclib_xicu_barrier_wait(cluster->arch.xicu, barrier_id);
}

error_t arch_barrier_destroy(struct cluster_s *cluster, uint_t barrier_id)
{
	return soclib_xicu_barrier_destroy(cluster->arch.xicu, barrier_id);
}
...
error_t arch_cpu_send_ipi(struct cpu_s *target)
{
	...
	return soclib_xicu_ipi_send(cluster->arch.xicu, target->lid);
}
